...
const id=33;
console.log(`Car 33 is a ${inventory[id-1]["car_year"]} ${inventory[id-1]["car_make"]} ${inventory[id-1]["car_model"]}` );

console.log("-----------------------------------------------------------------------------------------------------------------------------");
// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = 0;
lastCar=inventory[49]["car_make"];
console.log(`Car Make :- ${lastCar}`);
lastCar=inventory[49]["car_model"];
console.log(`Car Model :- ${lastCar}`);
...
let j=0,temp="",tempi="",tempj="",i=0;
for(let i=0;i<len;i++)
  {            
    for(let j=i+1;j<len;j++)
      {
        tempi=inventory[i]["car_model"].charCodeAt(0);
        tempj=inventory[j]["car_model"].charCodeAt(0);
        if(tempi>tempj)
          {
            temp=inventory[i]["car_model"];
...
let oldCars =[],count=0;
console.log("Car Year more than 2000 :-");
for(i=0;i<carYears.length;i++)
  {
    if(carYears[i]>2000)
     ...
     else(carYears[i]<2000)
     {
       delete carYears[i];
     }     
  } 
  console.log(`${count} cars are older than 2000 year`);//excluding 2000 year
...
for(let i=0;i<inventory.length;i++)
  {
    let bmwaudi=inventory[i]["car_make"];
    if(bmwaudi==="BMW")
     {      
      BMWAndAudi.push(inventory[i]);    
     }    
    else if(bmwaudi==="Audi")
     {        
      BMWAndAudi.push(inventory[i]);       
     }
     else
  ...
  let ans=JSON.stringify(BMWAndAudi);  
  console.log(ans); 
  console.log("-----------------------------------------------------------------------------------------------------------------------------");
